...
| `libphonenumber-js` | **1.13.10** | **Normalise l'identifiant saisi à `R0` en E.164** (FR-001, FR-002) — sous-ensemble **`/min`**, métadonnées réduites, suffisantes pour un numéro dont le pays est connu ; l'indicatif par défaut vient de la **configuration d'établissement** (`indicatif_telephonique_defaut`), jamais deviné. Aucune dépendance présente ne connaît un plan de numérotation, et l'écrire à la main réimplémenterait le plan ivoirien — **dont les mobiles sont passés à dix chiffres en 2021** —, sans rien dire du second pays visé. MIT · **aucune dépendance** · **aucune `peerDependency`** · `https://registry.npmjs.org/libphonenumber-js/latest`, **2026-08-07** |
...
| `vue-eslint-parser` | **10.4.1** | Parseur des blocs `template`. **Pair EXIGÉ** de `@intlify/eslint-plugin-vue-i18n` (`^10.0.0`). Il arrive transitivement avec `eslint-plugin-vue`, mais **une dépendance transitive n'est pas une dépendance déclarée** et P-03 refuse ce qu'aucun manifeste ne porte · `https://registry.npmjs.org/vue-eslint-parser/latest`, **2026-08-07** |
| `yaml-eslint-parser` | **2.1.0** | **Pair EXIGÉ** de `@intlify/eslint-plugin-vue-i18n` (`^1.2.2 \|\| ^2.0.0`). ⚠️ **Le point que `research.md` §5 laissait ouvert est tranché ici sur CONSTAT** : `peerDependenciesMeta` vaut `null` sur le document du registre, donc le pair n'est **pas optionnel** — et « probablement inutile parce que nos catalogues sont en TypeScript » n'était pas une version vérifiée · `https://registry.npmjs.org/yaml-eslint-parser/latest`, **2026-08-07** |
| `jsonc-eslint-parser` | **3.2.0** | **Pair EXIGÉ** du même greffon (`^2.3.0 \|\| ^3.0.0`). Même constat, même motif · `https://registry.npmjs.org/jsonc-eslint-parser/latest`, **2026-08-07** |
| `vue-tsc` | **3.3.9** | **Rend `pnpm typecheck` réel** — `nuxt typecheck` appelle `vue-tsc` nommément, et sans lui la commande déclarée s'arrêtait en réclamant son installation. ⚠️ **Un script déclaré qui ne peut pas s'exécuter est pire qu'un script absent** : il se lit comme un contrôle existant. `typescript` seul ne lit pas les blocs `template` d'un composant monofichier — `tsc` compile le script et ignore le reste. `peerDependencies.typescript` vaut `>=5.0.0`, satisfaite par `5.9.3` · `https://registry.npmjs.org/vue-tsc/latest`, **2026-08-07** |
| `jiti` | **2.7.0** | Chargeur TypeScript d'ESLint — sans lui, `eslint.config.ts` n'est pas lu et **la configuration ne charge pas du tout**. Présent transitivement dans l'arbre de Nuxt ; déclaré ici pour la même raison que `vue-eslint-parser` · `https://registry.npmjs.org/jiti/latest`, **2026-08-07** |
...
| `openapi-typescript` | **7.13.0** ⏳ phase 3 | **Génère les types TS depuis `openapi.json`** — le seul artefact généré (principe I·a), soumis à la porte du client généré. **Pas encore installé** : il n'y a aucun contrat OpenAPI à lire avant la phase 3, et on n'épingle pas dans un manifeste ce qu'on n'installe pas |
| `openapi-fetch` | **0.17.0** ⏳ phase 3 | Client fetch typé, ~6 kB — **écrit à la main, jamais généré**. **Pas encore installé**, même motif |
...
| **Numéros de téléphone** *(nouveau — cycle F2)* | **`libphonenumber-js`** (sous-ensemble `/min`) | `google-libphonenumber` — **~550 ko**, portage Java par Closure Compiler embarquant son propre système de modules : le paquet livré à l'Android 2 Go d'Aminata n'a pas cette place. `awesome-phonenumber` — enveloppe du précédent, même poids et une indirection de plus. **Toute validation maison** — les soixante lignes qu'on écrit pour éviter une bibliothèque, et qui figeraient sans le dire un plan de numérotation qui a déjà changé une fois |
| **Date et heure (JS)** *(nouveau — cycle F2)* | **`Intl` natif — AUCUNE DÉPENDANCE**, enveloppé dans `app/core/format/instant.ts`, la seule fonction qui écrit une heure ou une date | `date-fns` — plusieurs centaines de modules pour trois formats, et **ses fuseaux exigent un second paquet** (`date-fns-tz`), donc deux briques. `dayjs` — fuseaux par greffon, même défaut. `luxon` — embarque ses propres données de locale, **redondantes avec `Intl`**. `moment` — en maintenance close par ses auteurs. ⚠️ Le besoin réel tient en trois lignes — une heure, une date longue, une durée relative — et `Intl.DateTimeFormat` fait les deux premières **avec l'option `timeZone`**, qui est le point dur ; il est natif dans les deux moteurs de P-04. **La règle 4 du §1 écarte elle-même l'ajout quand ce qui est présent suffit** |

| **Planning à granularité horaire** *(nouveau — cycle F3)* | **AUCUNE BIBLIOTHÈQUE** — un calcul de largeur par heure dans `app/core/planning/ruban.ts`, fonction pure. *Tranché par le cycle **F3**, 2026-08-08* | `vis-timeline`, `frappe-gantt`, `dhtmlx-gantt`, `svelte-gantt` et **tout composant de gantt**. ⚠️ **Motif structurel, pas de poids** : tous supposent que **l'axe du temps est linéaire**. Sous cette hypothèse, un passage de 3 h sur une semaine occupe **1,8 %** de la largeur — ≈ 19 px sur 1 040, illisible. C'est **exactement le défaut que `V1` existe pour éviter** (*« passages de 1 à 4 h lisibles, pas écrasés dans une case de journée »*), et ce sont les formules infra-journalières qui font le volume du marché. Adopter l'une d'elles reviendrait à **payer une dépendance pour obtenir le comportement qu'on refuse**. Le ruban élastique ne déforme que **les vides** — les nuits mortes et les jours lointains se replient, les durées restent comparables entre elles ; une échelle logarithmique appliquée à un gantt existant les rendrait incomparables. Motif complet : `specs/005-reception-passage-note-planning/research.md` §1.2 |
...
| **10 ajouts · 0 montée · 0 famille nouvelle** | 2026-08-07 | **Cycle F1 — l'implémentation.** Le premier `package.json` du dépôt existe : **vingt-neuf dépendances, toutes en version EXACTE**, `pnpm-lock.yaml` commité, et **P-03 les vérifie dans les deux sens**. Les six ajouts planifiés sont installés (`vite-plugin-pwa`, `uuid`, `idb`, `knip`, `@vitest/coverage-v8`, `@intlify/eslint-plugin-vue-i18n`) ; **quatre de plus ont été constatés À L'INSTALLATION et non supposés** : `vue-eslint-parser 10.4.1`, `yaml-eslint-parser 2.1.0` et `jsonc-eslint-parser 3.2.0` sont des pairs **EXIGÉS** du greffon i18n — `peerDependenciesMeta` vaut `null`, ce que `research.md` §5 laissait ouvert ; `jiti 2.7.0` est ce sans quoi `eslint.config.ts` **n'est pas lu du tout**. Le second point ouvert est tranché lui aussi : `@vitest/browser` est marqué **optionnel**, rien à ajouter. **Aucune montée**, donc aucune montée majeure à signaler. ⚠️ **Écart déclaré** : le format JSON n'admet pas de commentaire, alors que la règle 4 du §1 en exige un au-dessus de chaque ligne. La justification vit dans le bloc `versionsJustification` du manifeste, et **le contrôle C7 de P-03 la vérifie DANS LES DEUX SENS** — ce qu'aucun commentaire ne permettrait. |
| **2 ajouts · 0 montée · 0 famille nouvelle** | 2026-08-07 | **Cycle F1 — la clôture, second ajout.** `vue 3.5.41` entre au manifeste : c'est un **pair EXIGÉ de `@vue/test-utils`**, et le montage d'un composant hors Nuxt échouait sur « Failed to resolve import "vue" ». Vue arrive transitivement avec Nuxt, mais **pnpm ne le lie pas à la racine** — et une dépendance transitive n'est pas une dépendance déclarée. ⚠️ **La version est ALIGNÉE SUR CELLE DE NUXT 4.5.1**, relevée dans le lockfile, **et non la dernière stable** : deux copies de Vue dans un même arbre casseraient l'identité des composants. **Condition de levée** : la montée de Nuxt, qui décidera de la valeur. Constatée en écrivant le test d'absence HTML de FR-050, qui exige un rendu réel — *un test qui ne monte rien ne prouve rien du HTML*. |
| **1 ajout · 0 montée · 0 famille nouvelle** | 2026-08-07 | **Cycle F1 — la clôture.** `vue-tsc 3.3.9` est ajoutée, et le motif vaut d'être écrit : **le script `typecheck` était déclaré et ne pouvait pas s'exécuter**. `nuxt typecheck` appelle `vue-tsc` nommément et s'arrêtait en réclamant son installation — c'est-à-dire qu'une commande du manifeste se lisait comme un contrôle existant alors qu'aucune ligne n'était vérifiée. *Un script déclaré qui échoue est pire qu'un script absent : le second se remarque.* Constaté en le lançant, au moment de rattacher les contrôles à `scripts/verifier.sh` (T053) ; il y entre comme quatrième préalable. Interrogée sur son registre le **2026-08-07**, `peerDependencies.typescript >=5.0.0` satisfaite par `5.9.3`. Aucune montée, donc aucune montée majeure à signaler. |
